...
## 10. Supabase Real-Time Cloud Synchronization & Database Architecture

### Overview
cassette.fm features a bi-directional cloud synchronization engine connecting frontend `localStorage` state with Supabase PostgreSQL tables backed by Google OAuth and Row Level Security (RLS).

### Tables & Schema (`supabase_schema.sql`)
1. **`public.liked_songs`**:
   - Primary Key: `(user_id, song_id)`
   - Columns: `user_id uuid references auth.users(id) on delete cascade`, `song_id text`, `song_data jsonb`, `created_at timestamptz default now()`.
   - RLS: Only the authenticated user (`auth.uid() = user_id`) can `SELECT`, `INSERT`, `UPDATE`, and `DELETE`.
2. **`public.user_playlists`**:
   - Primary Key: `id text`
   - Columns: `id text`, `user_id uuid references auth.users(id) on delete cascade`, `name text`, `description text`, `songs jsonb`, `thumbnail text`, `created_at timestamptz`, `updated_at timestamptz`.
   - RLS: Full CRUD restricted to `auth.uid() = user_id`.
3. **`public.playback_history`**:
   - Primary Key: `id bigint generated by default as identity`
   - Columns: `id`, `user_id uuid references auth.users(id) on delete cascade`, `song_id text`, `song_data jsonb`, `played_at timestamptz default now()`.
